...
module.exports = {
    data: {
        name: "pop",
        description: "To toggle enable/disable the pop filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.pop) {
            player.setPop(false);
            return await intReply(interaction, `Pop filter/effect is now disabled.`, color);
        } else {
            player.setPop(true);
            return await intReply(interaction, `Pop filter/effect is now enabled.`, color);
        };
