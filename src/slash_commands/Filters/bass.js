...
module.exports = {
    data: {
        name: "bass",
        description: "To toggle enable/disable the bass filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.bass) {
            ...
            return await intReply(interaction, `Bass filter/effect is now disabled.`, color);
        } else {
            player.setBass(true);
            return await intReply(interaction, `Bass filter/effect is now enabled.`, color);
        };
    }
}