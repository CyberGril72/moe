...
module.exports = {
    data: {
        name: "electronic",
        description: "To toggle enable/disable the electronic filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.electronic) {
            ...
            return await intReply(interaction, `Electronic filter/effect is now disabled.`, color);
        } else {
            player.setElectronic(true);
            return await intReply(interaction, `Electronic filter/effect is now enabled.`, color);
        };
    }
}