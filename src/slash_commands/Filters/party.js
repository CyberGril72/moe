...
module.exports = {
    data: {
        name: "party",
        description: "To toggle enable/disable the party filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.party) {
            ...
            return await intReply(interaction, `Party filter/effect is now disabled.`, color);
        } else {
            player.setParty(true);
            return await intReply(interaction, `Party filter/effect is now enabled.`, color);
        };
