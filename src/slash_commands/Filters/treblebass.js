...
module.exports = {
    data: {
        name: "treblebass",
        description: "To toggle enable/disable the treblebass filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.treblebass) {
            ...
            return await intReply(interaction, `Treblebass filter/effect is now disabled.`, color);
        } else {
            player.setTreblebass(true);
            return await intReply(interaction, `Treblebass filter/effect is now enabled.`, color);
        };
    }
}