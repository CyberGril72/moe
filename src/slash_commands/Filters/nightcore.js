...
module.exports = {
    data: {
        name: "nightcore",
        description: "To toggle the nightcore effect/filter to the player."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.nightcore) {
            ...
            return await intReply(interaction, `Nightcore filter/effect is now disabled.`, color);
        } else {
            player.setNightCore(true);
            return await intReply(interaction, `Nightcore filter/effect is now enabled.`, color);
        };
    }
}