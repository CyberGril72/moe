...
module.exports = {
    data: {
        name: "radio",
        description: "To toggle enable/disable the radio filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.radio) {
            ...
            return await intReply(interaction, `Radio filter/effect is now disabled.`, color);
        } else {
            player.setRadio(true);
            return await intReply(interaction, `Radio filter/effect is now enabled.`, color);
        };
    }
}