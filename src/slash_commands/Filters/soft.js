...
module.exports = {
    data: {
        name: "soft",
        description: "To toggle enable/disable the soft filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.soft) {
            ...
            return await intReply(interaction, `Soft filter/effect is now disabled.`, color);
        } else {
            player.setSoft(true);
            return await intReply(interaction, `Soft filter/effect is now enabled.`, color);
        };
    }
}