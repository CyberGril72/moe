...
module.exports = {
    data: {
        name: "rock",
        description: "To toggle enable/disable the rock filter."
    },
    ...
    execute: async (client, interaction, color) => {
        ...
        if(player.rock) {
            ...
            return await intReply(interaction, `Rock filter/effect is now disabled.`, color);
        } else {
            player.setRock(true)
            return await intReply(interaction, `Rock filter/effect is now enabled.`, color);
        };
    }
}